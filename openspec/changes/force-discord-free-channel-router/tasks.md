...
- [x] 1.3 Force matched Discord free-response turns onto the local router `coding` path before agent creation instead of relying on upstream session-switch behavior.
...
- [x] 3.1 Add or extend managed runtime validation to prove Discord free-response sessions stay pinned to `http://127.0.0.1:8788/v1` with the `coding` alias.
