...
**Reason**: The runtime no longer maintains per-profile liveness markers for a fleet of named profile gateways.
**Migration**: Publish one managed `gateway.pid` at the root managed Hermes home for the single gateway service.

### Requirement: Managed gateway liveness markers follow the service lifecycle
**Reason**: Service lifecycle alignment still matters, but it now applies to one managed gateway service and one marker file.
**Migration**: Keep the same start/restart/stop guarantees for the root-managed `gateway.pid`.
