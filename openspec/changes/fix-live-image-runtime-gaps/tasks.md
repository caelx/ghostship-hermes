...
- [ ] 1.1 Update the managed Hermes config scaffold so the image ships `opencode-go/minimax-m2.7` as primary and router alias `agentic` as fallback.
- [ ] 1.2 Keep `GHOSTSHIP_ROUTER_DISABLED_MODELS=openrouter/free` as the managed router default and verify router fallback auth still uses `OPENAI_API_KEY`.

## 2. Dashboard Runtime Validation Surface

- [ ] 2.1 Extend dashboard status parsing to expose both primary and fallback agent config details from the managed Hermes config.
- [ ] 2.2 Update the dashboard UI and fixtures so operators can see the managed primary/fallback contract and liveness markers directly.
...
- [ ] 3.2 Remove the avoidable cold-start healthcheck race so the first failing probe reflects a real dashboard outage.
...
- [ ] 5.1 Update `README.md`, `docs/runtime-env.md`, `AGENTS.md`, and `CHANGELOG.md` to match the final live-image contract and validation flow.
