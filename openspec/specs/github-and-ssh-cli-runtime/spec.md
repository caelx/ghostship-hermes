...
#### Scenario: Default image exposes `gh`, `ssh`, `scp`, and `ssh-keygen`
- **WHEN** the default Hermes image is built from the repo flake
- **THEN** the image contents include the `gh` executable
- **AND** the image contents include the `ssh`, `scp`, and `ssh-keygen` executables
...
The repo's runtime policy and operator guidance SHALL describe `gh` and the OpenSSH client tools, including SSH key generation through `ssh-keygen`, as approved non-`ghostship-*` CLIs in the default image.
...
- **AND** the documented approved non-`ghostship-*` CLI set includes the OpenSSH client tools needed for `ssh`, `scp`, and `ssh-keygen`
