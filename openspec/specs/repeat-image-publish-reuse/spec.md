# repeat-image-publish-reuse Specification
...
The publish workflow SHALL derive an immutable per-architecture final-image identifier from the reusable base-image tag plus the `ghostship-hermes-overlay-bundle` derivation that define the GitHub publication path, and SHALL check GHCR for that image before starting a rebuild.

#### Scenario: Immutable final image already exists
- **WHEN** the publish workflow evaluates an architecture whose reusable base-image tag and `ghostship-hermes-overlay-bundle` derivation already match a published immutable final image in GHCR
...
- **WHEN** the publish workflow evaluates an architecture whose reusable base-image tag and `ghostship-hermes-overlay-bundle` derivation do not match any published immutable final image in GHCR
- **THEN** the workflow falls back to the normal layered base-plus-overlay build path for that architecture
