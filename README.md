...
- recency bias: newer models get a strong score lift after free-only and capability filters, but alias-specific family priors still dominate the final ordering
- parameter bias: `vision` gets a strong larger-is-better size bonus, while `auxiliary` gets a smaller-is-better size penalty so helper lanes stay compact
