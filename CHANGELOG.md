...
- Replaced the bundled Bitwarden Password Manager CLI `bw` with the Bitwarden Secrets Manager CLI `bws`, pinned a Hermes-managed `BWS_CONFIG_FILE` and `state_dir` under `~/.hermes/bws`, rewrote the repo `bitwarden` skill and docs around machine-account project secrets, and added `ghostship-changedetection` with the mirrored stable upstream OpenAPI snapshot, full API CLI coverage, and a repo-managed changedetection workflow skill.
