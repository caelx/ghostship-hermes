...
- Reworked the dashboard into a multi-terminal tab UI that keeps the old Hermes logo, serves directly from the controller process, opens each browser terminal at `/home/hermes`, creates focused tabs immediately with a loading state, labels sessions from the shell cwd or current command, allows unlimited ephemeral `ttyd` sessions, and returns to a blank home state when all tabs are closed.
