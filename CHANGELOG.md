...
- Made `packages/hermes-dashboard` the canonical packaged MMX dashboard artifact, exposed it as a direct flake package, removed the stale image runtime asset-root seam, updated the dashboard smoke test and docs to the MMX UI contract, and wrapped the packaged entrypoint with the `ttyd` runtime it needs for `NEW_UNIT` terminal launches, while hardening ttyd port selection so stale listeners do not hijack new dashboard sessions.
