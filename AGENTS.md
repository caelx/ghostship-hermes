...
- Keep one persistent managed gateway service, `hermes-gateway.service`, running as an upstream-style `systemd --user` service for `hermes` with repo-owned boot wiring that starts it automatically.
...
- Hermes health/status validation must also recognize the wrapped managed gateway argv (`.hermes-wrapped gateway run --replace`); otherwise commands such as `hermes doctor` can decide the live service is invalid and delete `gateway.pid` even while `hermes-gateway.service` is still active.
- Upstream Hermes does not expose this single-agent repo contract directly, so keep the repo-owned bootstrap, env rewrite, and seed staging in NixOS-managed boot units while aligning the gateway itself to upstream `systemd --user` `hermes-gateway.service` instead of a repo-owned system unit.
...
- When the image manages `hermes-gateway.service` through `systemd --user`, bootstrap must also surface that unit at `/home/hermes/.config/systemd/user/hermes-gateway.service` and Hermes CLI must treat the managed `/etc/systemd/user` unit as current, or upstream `hermes gateway status` falls back to manual/outdated warnings.
...
- Hermes 0.9 upstreams the OpenCode Go doctor handling and native custom-provider plumbing, so repo wrapper maintenance should not keep old doctor/provider compatibility shims once the upstream release already covers them.
- Hermes 0.9 still does not provide a declarative "pin this Discord channel to the local router alias" control; keep the repo-owned gateway patch for the managed Discord free-response channel until upstream adds a real per-channel routing override that beats session-scoped `/model` state.
...
- The pinned `nixcache-oci` planner identifies upload candidates from `nix build --dry-run`, so `publish-image` must capture the cache upload plan before the real build starts; planning after a successful build produces zero paths, leaves `cache-index` missing, and makes later runs look uncached even though cache publication "succeeded."
- Shared-cache publication is a separate maintenance path, not part of every image publish: normal push builds may consume `caelx/ghostship-cache` when `cache-index` exists, but only the daily scheduled `publish-image` run or an explicit `workflow_dispatch` with `publish_shared_cache=true` should attempt cache uploads. If uploads return `HTTP 403`, treat the cache as unseeded and fix the GHCR token/package permissions instead of assuming the cache was refreshed. Prefer the job `GITHUB_TOKEN` path before introducing a custom GHCR token override.
