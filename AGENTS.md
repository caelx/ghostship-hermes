...
- Hermes `doctor` only marks the `messaging` toolset available when `gateway.status` sees a live profile gateway via `gateway.pid`; projecting Discord env into profile `.env` is necessary to configure the gateway, but it is not sufficient to make the warning disappear if the gateway never starts or has already failed.
- Managed gateway services should own `~/.hermes/profiles/<profile>/gateway.pid` themselves: write the full JSON liveness record for the current service PID before `exec`, remove stale pidfiles before start and after stop, and treat `gateway_state.json` as informational rather than the primary liveness contract.
...
- The image should bootstrap `assistant`, `operations`, and `supervisor` profiles so operators can inspect the upstream `~/.hermes/profiles/...` layout immediately after boot, with `assistant` set as the sticky default profile, and repo-owned assistant runtime checks should use `hermes -p assistant` explicitly instead of relying on `~/.hermes/active_profile`.
- The bootstrap oneshot should materialize the approved `assistant`, `operations`, and `supervisor` profiles, preserve shared and per-profile skill copy-once behavior, seed each profile `SOUL.md` from `/home/hermes/seeds/profiles/<profile>/SOUL.md`, replace the old Hermes-generated generic prompt during migration, and keep future seed updates in sync only while the live `SOUL.md` still matches the last seeded hash so operator or agent edits remain authoritative. It should also write the managed runtime env into each profile `~/.hermes/profiles/<profile>/.env` and leave config generation shell-only without depending on extra Python packages like PyYAML.
- Bootstrap should rewrite managed profile `.env` files atomically because those same paths are watched by the gateway restart units; rewriting the watched file in place can restart a gateway against a partially written `.env`.
...
- Profile `.env` should hold only the profile-facing runtime contract: Hermes/provider secrets, browser config, Discord settings, Bitwarden access, and every utility/service env that installed `ghostship-*` CLIs or router-invoked utility calls need at runtime. Keep image infrastructure, router-daemon internals, container boot plumbing, and test-only utility headers outside profile `.env`.
...
- The scaffolded profile `config.yaml` now includes Discord defaults (`require_mention = true`, `auto_thread = false`, `reactions = true`, `group_sessions_per_user = true`), while each gateway service maps profile-specific env vars into Hermes' standard Discord env names so a shared `DISCORD_GENERAL_CHANNEL_ID` stays mention-only and each `DISCORD_<PROFILE>_CHANNEL_ID` becomes a free-response role channel without spawning automatic Discord threads. Hermes still has no native per-profile Discord icon setting; distinct icons require separate Discord bot applications.
- Tirith is intentionally installed in the image runtime and kept on the service PATH so Hermes pre-exec scanning can use `tirith_path = "tirith"` without a bootstrap download. The image also vendors `agent-browser` without preinstalling Chrome or Chromium; the shared scaffold sets `browser.cloud_provider = "local"` so Hermes local browser mode defaults to that CLI unless an operator manually attaches CDP with `/browser connect`. The managed three-profile runtime treats remote browser CDP as profile-scoped operator configuration: `BROWSER_ASSISTANT_CDP_URL`, `BROWSER_OPERATIONS_CDP_URL`, and `BROWSER_SUPERVISOR_CDP_URL` are the container-level source vars, and bootstrap must translate each one into only that profile's local `BROWSER_CDP_URL`.
...
- Dependency-audit rule for the image split: if the realized overlay still carries shared non-Ghostship closures used by the runtime or approved utility set, move those closures into the base image until the overlay store paths are limited to Ghostship-owned packages plus the small overlay assembly env. The current approved base-side extras are the shared Python service deps (`httpx`, `typer`, `fastapi`, `uvicorn`, `websockets`) and the stable external utility closures (`agent-browser`, `bws`, `gcloud`, `gws`).
...
- The publish workflow should derive reusable `ghostship-hermes-base` tags from tracked base-affecting inputs rather than the raw base-image derivation path; derivation-path tags are too sensitive and force unnecessary base rebuilds on overlay-only repo changes.
