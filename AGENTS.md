...
- GitHub publication should build the explicit `ghostship-hermes-image` bundle inside a running `ghostship-hermes-base` container so the publish job reuses the base image's baked `/nix/store`; pulling the base image without moving the Nix build into that container only helps the later Docker import step and will not materially speed content builds.
