...
- GitHub publication should always rebuild the explicit `ghostship-hermes-image` bundle; do not gate publishes on content-addressed final-image reuse. Instead, keep one rolling Hawaii-day full-image cache tag per arch (`daily-<hermes-release>-<YYYY-MM-DD>-<arch>`), refresh it after every successful publish, and let same-day publishes build inside that full image. A new `hermes-release.txt` value automatically starts a fresh daily line, and `workflow_dispatch.force_full_build=true` remains the manual bypass when you want a clean full rebuild on demand.
