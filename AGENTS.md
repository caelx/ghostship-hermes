...
- Alias-specific family priors should strongly bias `coding`, `agentic`, and `auxiliary` after capability and free-only filters, recency bias should be strong enough to lift newer families instead of acting only as a weak tie-breaker, `vision` should prefer larger models when capability fit is otherwise equal, and `auxiliary` should prefer smaller helper models.
