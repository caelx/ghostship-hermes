...
- Profile `.env` should hold only the profile-facing runtime contract, including Hermes/provider secrets, browser config, Discord settings, Bitwarden access, and operator-facing `ghostship-*` CLI env that the profile itself uses. Keep image infrastructure, router-daemon internals, and container boot plumbing outside profile `.env`.
