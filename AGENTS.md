...
- Managed gateway services should own `~/.hermes/profiles/<profile>/gateway.pid` themselves: write the current service PID before `exec`, remove stale pidfiles before start and after stop, and treat `gateway_state.json` as informational rather than the primary liveness contract.
