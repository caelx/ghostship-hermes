...
- GitHub publication should always rebuild the explicit `ghostship-hermes-image` bundle on the runner host; do not gate publishes on content-addressed final-image reuse and do not reintroduce the abandoned daily-image in-container build path. If free Nix package caching is reconsidered later, treat it as a real binary-cache layer, not an OCI parent-image trick.
