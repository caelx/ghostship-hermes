...
- GitHub publication should always rebuild the explicit `ghostship-hermes-image` bundle; do not gate publishes on content-addressed final-image reuse. Instead, keep one rolling daily full-image cache tag per arch (`daily-<hermes-release>-<YYYY-MM-DD>-<arch>`), refresh it after every successful publish, and let same-day publishes build inside that full image. The date portion uses UTC, a new `hermes-release.txt` value automatically starts a fresh daily line, and `workflow_dispatch.force_full_build=true` remains the manual bypass when you want a clean full rebuild on demand.
- Raw imported NixOS image layers are not booted systems: when CI runs `nix build` inside a pulled `ghostship-hermes` daily image, the helper must discover a real store-path shell and `nix`, export the image CA bundle, force `NIX_REMOTE=local`, clear inherited `build-hook` / `builders` settings, and provide the needed Nix shared-library dirs via `LD_LIBRARY_PATH`; otherwise nested `nix` processes can fail on `libnixcmd.so` even though the top-level shell starts.
