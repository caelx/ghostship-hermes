...
- Router shortlist pins may use repo-friendly `openrouter/` or `opencode/` prefixes, but backend dispatch must normalize those prefixes away so provider calls use the provider's real model id.
