...
- Shared CLI contract rule: every `ghostship-*` invocation must honor a default hard timeout of `30` seconds via `--timeout`, use the shared JSON error/exit-code envelope, and expose `--dry-run` on write/delete commands so agents can inspect the exact request object before mutating remote state.
- Shared transport lesson: `BaseHttpClient.request_json()` must call the base transport request path directly (`BaseHttpClient.request(self, spec)`), not a service override like `self.request(spec)`, otherwise clients that expose their own `request(method, path, ...)` helper work in unit doubles but fail against real services.
