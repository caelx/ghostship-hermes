...
- Alias-specific family ordering should bias `coding`, `agentic`, and `auxiliary` after capability and free-only filters by applying relative rank bonuses only to families that are actually present, recency bias should be strong enough to lift newer families instead of acting only as a weak tie-breaker, exact id/name family matches must outrank description-only hints, `coding`/`agentic`/`vision` should only penalize smaller family variants when a larger sibling exists, and `auxiliary` should prefer smaller helper models.
