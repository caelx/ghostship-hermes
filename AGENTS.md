...
- The bootstrap oneshot should materialize the approved `assistant`, `operations`, and `supervisor` profiles, preserve shared and per-profile skill plus `SOUL.md` copy-once behavior, write only the shared Hermes `.env` for the small set of provider secrets the scaffold currently needs, and leave config generation shell-only without depending on extra Python packages like PyYAML.
