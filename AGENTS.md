...
- Raw imported NixOS image layers are not booted systems: when CI runs `nix build` inside a pulled `ghostship-hermes` daily image, the helper must discover a real store-path shell and a concrete store `nix`, export the image CA bundle, force `NIX_REMOTE=local`, clear inherited `build-hook` / `builders` settings, and put that `nix` binary's own `bin` dir first on `PATH`; do not spray wildcard Nix/OpenSSL lib dirs into `LD_LIBRARY_PATH`, because mixed store eras can break the in-image build with loader mismatches even after `libnixcmd.so` is fixed.
