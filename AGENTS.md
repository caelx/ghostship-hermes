...
- Optional runtime skill staging lives under `/home/hermes/seeds/skills/<skill>`; bootstrap may copy a missing skill directory only into `/home/hermes/.hermes/skills/<skill>`, and it must never overwrite an existing destination skill because Hermes owns it after first seed.
...
- The current single-agent scaffold expects the local router alias `coding` as the primary model path via `model.provider = auto` plus `model.base_url = http://127.0.0.1:8788/v1`, `OPENCODE_GO_API_KEY` for the Hermes-native `opencode-go/minimax-m2.7` fallback, `GOOGLE_AI_STUDIO_API_KEY` for all direct auxiliary tasks, and `BWS_ACCESS_TOKEN` only for Bitwarden-backed workflow secrets.
...
- Keep `ghostship-hermes-image` as the explicit local export and image-loading bundle contract, but keep GitHub GHCR publication on the layered `ghostship-hermes-base` plus `ghostship-hermes-overlay-bundle` path so content publishes can reuse the base image and avoid rebuilding the full final rootfs derivation.
