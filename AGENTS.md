...
- The current public router alias set is `auxiliary`, `coding`, `vision`, and `tts`; Hermes root plus both managed profiles should default to `coding`, while `auxiliary` remains an extra alias rather than the default runtime lane.
...
- Coding-family priors should strongly bias the coding alias after capability and free-only filters, recency bias should be meaningful enough to lift newer families rather than acting only as a weak tie-breaker, and model-size parameter bonuses should matter most for `vision` so larger Gemma-class models sort ahead of smaller ones when capability fit is otherwise equal.
