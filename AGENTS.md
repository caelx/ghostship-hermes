...
- Dashboard status is an operator-facing validation surface: keep it exposing the managed primary model, fallback model, endpoint wiring, and `gateway.pid` state so live image checks can verify the runtime contract without shelling into the container first.
