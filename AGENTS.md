...
- The wrapper at `/usr/local/bin/google-chrome` must inject CloakBrowser's default stealth args on each launch, default to `/home/hermes/.local/state/cloakbrowser` only when callers omit `--user-data-dir`, map agent-browser temp profile dirs into persisted per-session dirs so concurrent launches do not collide on Chromium `SingletonLock`, and default Chrome to `--log-level=3` unless the caller supplies a log level.
