...
- The image should bootstrap `assistant`, `operations`, and `supervisor` profiles so operators can inspect the upstream `~/.hermes/profiles/...` layout immediately after boot, and repo-owned assistant runtime checks should use `hermes -p assistant` explicitly instead of relying on `~/.hermes/active_profile`.
