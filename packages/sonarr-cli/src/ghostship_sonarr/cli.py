...
def get_client() -> SonarrClient:
    ...
    if not base_url or not api_key:
        print("Error: SONARR_URL and SONARR_API_KEY environment variables must be set.", file=sys.stderr)
...
@app.command()
def get_series(series_id: int, pretty: bool = typer.Option(False, "--pretty", help="Pretty print JSON output")):
...
@app.command()
def add(tvdb_id: int, title: str, quality_profile_id: int = 1, language_profile_id: int = 1, root_folder_path: str = "/tv", monitored: bool = True, pretty: bool = typer.Option(False, "--pretty", help="Pretty print JSON output")):
