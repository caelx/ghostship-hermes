...
def get_client() -> PyLoadClient:
    ...
    if not base_url or not username or not password:
        print("Error: PYLOAD_URL, PYLOAD_USER, and PYLOAD_PASS environment variables must be set.", file=sys.stderr)
        raise typer.Exit(code=1)
    
    return PyLoadClient(base_url, username, password)

@app.command()
def status(pretty: bool = typer.Option(False, "--pretty", help="Pretty print JSON output")):
...
@app.command()
def downloads(pretty: bool = typer.Option(False, "--pretty", help="Pretty print JSON output")):
...
@app.command()
def pause(pretty: bool = typer.Option(False, "--pretty", help="Pretty print JSON output")):
