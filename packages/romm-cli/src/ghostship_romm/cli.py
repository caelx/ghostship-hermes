...
def get_client() -> RommClient:
    ...
    if not base_url or not token:
        print("Error: ROMM_URL and ROMM_TOKEN environment variables must be set.", file=sys.stderr)
...
@app.command()
def list_roms(page: int = 1, page_size: int = 24, platform: Optional[str] = None, pretty: bool = typer.Option(False, "--pretty")):
...
@app.command()
def scan(library_id: Optional[int] = typer.Option(None, "--id", help="Library ID to scan"), pretty: bool = typer.Option(False, "--pretty")):
