...
class SynologyClient:
    ...
    def _request(self, api: str, method: str, version: int, path: str, params: Optional[Dict[str, Any]] = None, files: Optional[Dict[str, Any]] = None) -> Any:
    ...
    def logout(self) -> bool:
        ...
        version = auth_info.get("maxVersion", 6)
        path = auth_info.get("path", "auth.cgi")
        self._request("SYNO.API.Auth", "logout", version, path, {"session": "FileStation"})
        self.sid = None
        return True
    ...
    def list_shares(self) -> Any:
        ...
        return self._request(api, "list_share", info.get("maxVersion", 2), info.get("path", "entry.cgi"))
    ...
    def list_files(self, folder_path: str, offset: int = 0, limit: int = 100, sort_by: str = "name") -> Any:
        ...
        return self._request(api, "list", info.get("maxVersion", 2), info.get("path", "entry.cgi"), params)

    def get_file_info(self, path: str) -> Any:
        api = "SYNO.FileStation.List"
        info = self.api_info.get(api, {})
        return self._request(api, "getinfo", info.get("maxVersion", 2), info.get("path", "entry.cgi"), {"path": path})
    ...
    def search_start(self, folder_path: str, pattern: str, recursive: bool = True) -> str:
        ...
        data = self._request(api, "start", info.get("maxVersion", 2), info.get("path", "entry.cgi"), params)
    ...
    def create_folder(self, folder_path: str, name: str, force_parent: bool = False) -> Any:
        ...
        return self._request(api, "create", info.get("maxVersion", 2), info.get("path", "entry.cgi"), params)
    ...
    def rename(self, path: str, name: str) -> Any:
        ...
        return self._request(api, "rename", info.get("maxVersion", 2), info.get("path", "entry.cgi"), params)
