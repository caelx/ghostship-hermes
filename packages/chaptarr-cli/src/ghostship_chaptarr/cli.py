...
app = typer.Typer(help='Typed Chaptarr Public API CLI.', no_args_is_help=True)
APP_STATE: dict[str, float] = {'timeout': DEFAULT_TIMEOUT}
...
def get_client() -> ChaptarrClient:
    base_url = require_env('CHAPTARR_URL', os.getenv('CHAPTARR_URL'))
    api_key = require_env('CHAPTARR_API_KEY', os.getenv('CHAPTARR_API_KEY'))
    api_path = os.getenv('CHAPTARR_API_PATH', 'api')
    api_version = os.getenv('CHAPTARR_API_VERSION', 'v1')
...
def request(
    ...
    client = get_client()
    normalized_path = client.normalize_path(raw_path)
    _run_write(
        ...
        lambda timeout: client.request_json(
            method,
            normalized_path,
            params=params,
            json_body=payload,
