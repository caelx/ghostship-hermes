...
@app.command()
def indexer_stats(pretty: bool = typer.Option(False, "--pretty")):
    """Get indexer statistics."""
    client = get_client()
    try:
        data = client.get_indexer_stats()
        echo_json(data, pretty=pretty)
    except Exception as e:
        print(f"Error fetching indexer stats: {e}", file=sys.stderr)
        raise typer.Exit(code=1)


@app.command()
def indexer_status(pretty: bool = typer.Option(False, "--pretty")):
    """Get indexer status."""
    client = get_client()
    try:
        data = client.get_indexer_status()
        echo_json(data, pretty=pretty)
    except Exception as e:
        print(f"Error fetching indexer status: {e}", file=sys.stderr)
        raise typer.Exit(code=1)
