...
docker run --rm   --entrypoint "$build_shell"   -v "$repo_root:/src:ro"   -v "$bundle_output_dir:/out"   -e FLAKE_ATTR="$flake_attr"   -e SYSTEM_PATH_BIN="$system_path_bin"   -e CERTIFICATE_BUNDLE="$certificate_bundle"   -e NIX_BINARY="$nix_binary"   "$build_image"   -lc '
    set -euo pipefail
    build_nix="$NIX_BINARY"
    build_nix_dir="${build_nix%/nix}"
    export PATH="$build_nix_dir:$SYSTEM_PATH_BIN:$PATH"
